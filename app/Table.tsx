            ...
            todo?.map((item, index)=>(
                <div className='flex w-[100%] py-2 px-4 bg-[#FFFFFF]'>
                    ...
                    <span className='w-[20%] flex gap-2 relative -z-0'>
                        <img src='https://cdn-icons-png.flaticon.com/512/10146/10146590.png' alt='' className='w-[20px] h-[20px] hover: cursor-pointer' onClick={()=>set(index)}/>
                        ...
                        {
                            (add && ind==index) ? (<Modal key={item._id} addtodo={setAdd} fetchTodo={fetchTodo} tit={"Edit Schecdule"} update={true} inde={item?._id}/>) : (<></>)
                        }
                        </div>
                    </span>
                </div>
